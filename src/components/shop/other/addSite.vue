<template>
    <div id="add_site">
        ...
        <div class="site_btn" @click="insertReceiverAddressWithTx">
            保存
        </div>
    </div>
</template>
...
<script>
    ...
    import {findProvCityAreaAll,insertReceiverAddressWithTx,findReceiverAddressAll} from '../../../api/getData';
    ...
    export default{
        ...
        methods:{
            ...
            //确认新增地址
            async insertReceiverAddressWithTx(){
                try {
                    ...
                    const res = await insertReceiverAddressWithTx(this.siteData);
                    if (res.data.code == 0) {
                        this.$router.go(-1);
                    } else {
